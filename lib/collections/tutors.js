...
Tutors.allow({
  update: function(tutorProfileId, profile) { return ownsProfile(tutorProfileId, profile); },
  remove: function(tutorProfileId, profile) { return ownsProfile(tutorProfileId, profile); },
});

// Tutors.allow({
//   updateSkill: function (tutorProfileId, tutorProfileEdits, skillProperties) { return }
// })
...
Meteor.methods({
  tutorProfileInsert: function(tutorProfileFields) {
    ...
    if(tutorProfileWithSameId) { // if it is undefined, evaluates to falsey (means profile doesnt exists)
      return {
